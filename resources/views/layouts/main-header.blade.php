							...
							<li class="">
								<div class="dropdown  nav-itemd-none d-md-flex">
									<a href="#" class="d-flex  nav-item nav-link pl-0 country-flag1" data-toggle="dropdown" aria-expanded="false">
										<span class="avatar country-Flag mr-0 align-self-center bg-transparent"><img src="{{URL::asset('assets/img/flags/images.png')}}" alt="img"></span>
									</a>
                                ...
                                <div class="dropdown-menu">
									<div class="menu-header-content bg-primary text-right">
										<div class="d-flex">
                                                {{-- <h6 class="dropdown-title mb-1 tx-15 text-white font-weight-semibold">
                                                    {{__('messagevalidation.users.notifications')}}
                                                </h6> --}}
									...
									<div class="main-notification-list Notification-scroll" id="unreadNotifications">
                                        @forelse (auth()->user()->unreadNotifications as $notification)
                                            @if ($notification->type == 'App\Notifications\postntf')
                                                <a class="d-flex p-3 border-bottom" href="{{ route('page_detailsposts', $notification->data['idpost'])}}">
                                                    <div class="mr-3">
                                                        <h5 class="notification-label mb-1">{{$notification->data['new']}} :  {{$notification->data['titlepost']}}</h5>
                                            ...
                                            @if ($notification->type == 'App\Notifications\productntf')
                                                <a class="d-flex p-3 border-bottom" href="{{ route('page_detailsproducts', $notification->data['idproduct'])}}">
                                                    <div class="mr-3">
                                                        <h5 class="notification-label mb-1">{{$notification->data['new']}} :  {{$notification->data['titleproduct']}}</h5>
                                                        <div class="notification-subtext">{{$notification->created_at->diffForHumans()}}</div>
                                                    </div>
                                                    <div class="mr-auto">
                                                        <i class="las la-angle-left text-left text-muted"></i>
									...
									<div class="main-header-profile bg-primary p-3">
										<div class="d-flex wd-100p">
											...
											<div class="mr-3 my-auto">
												<h6>{{Auth::user()->profileuser->firstname}} {{Auth::user()->profileuser->lastname}} </h6><span>Premium Member</span>
											</div>
										</div>
									</div>
									<a class="dropdown-item" href="{{ route('profile.editbackend') }}"><i class="bx bx-user-circle"></i> {{__('message.Profile')}} </a>
									<a class="dropdown-item" href=""><i class="bx bx-envelope"></i>Messages</a>
									<a class="dropdown-item" href=""><i class="bx bx-slider-alt"></i> Account Settings</a>
									<a class="dropdown-item" href="{{ route('auth.destroybackend') }}"><i class="bx bx-log-out"></i> {{__('message.signout')}}</a>
								</div>
							</div>
