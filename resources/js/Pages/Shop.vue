...
const props = defineProps({
    ...
    const sizeheight = ref(props.filtres.sizeheight ?? "")
    const sizewidth = ref(props.filtres.sizewidth ?? "")
    ...
    const search = _.throttle(function(){
        ...
        console.log("sizewidth : ", sizewidth.value)
    ...
    // Wishlist
        ...
        function addwishlist(id) {
            formWishlist.get(route('addwishlist', id),
                {
                    onSuccess: (page) => {
                        useSwallSuccess("Add In Wish List Success")
                    },
                    onerror: (errors) => {
...
<template>
    ...
    <div class="container grid lg:grid-cols-4 gap-6 pt-4 pb-16 items-start relative">
        ...
        <div class="col-span-1 bg-white px-4 pt-4 pb-6 shadow rounded overflow-hidden absolute lg:static left-4 top-16 z-10 w-72 lg:w-full lg:block">
            <div class="divide-gray-200 divide-y space-y-5 relative" :class="{ block: showingNavigationDropdown, hidden: !showingNavigationDropdown }">
                <!-- category filter -->
                <div class="pt-4">
                ...
                <div class="pt-4">
                    <h3 class="text-xl text-gray-800 mb-3 uppercase font-medium">{{__('CATEGORIES')}}</h3>
                    <!-- <div class="space-y-2"> -->
                        <!-- single category -->
            ...
            <div v-if="props.products.data.length">
                <div class="grid lg:grid-cols-2 xl:grid-cols-3 sm:grid-cols-2 gap-6 mt-12">
                    ...
                    <div class="group rounded bg-white shadow overflow-hidden" v-for="product in props.products.data" :key="product.id">
                        ...
                        <!-- product content end -->
                        <!-- product button -->
                        <!-- <button @click="addwishlist(product.id)" class="block w-full py-1 text-center text-white bg-primary border border-primary rounded-b hover:bg-transparent hover:text-primary transition" > Add to Cart </button> -->
